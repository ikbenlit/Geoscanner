...
export function analyzeMultimodal(
  ...
  if (transcriptsAnalysis.totalMediaElements > 0) {
    const transcriptScore = transcriptsAnalysis.withTranscript / transcriptsAnalysis.totalMediaElements;
  ...
  if (contentStructureAnalysis.hasTables && contentStructureAnalysis.tableWithHeaders) structureScore += 0.75;
...
function analyzeAltText(html: string): MultimodalResult['details']['altText'] {
  ...
  const poorQualityPhrases = [
    'image', 'picture', 'afbeelding', 'foto', 'plaatje', 'graphic',
...
function analyzeTranscripts(html: string): MultimodalResult['details']['transcripts'] {
  ...
  const transcriptSectionRegex = /(?:<div[^>]*class=["'][^"']*transcript[^"']*["'][^>]*>|<details[^>]*>.*?<summary[^>]*>.*?transcript)/gi;
